...
resource "aws_security_group_rule" "client_ecs_egress_rds" {
  ...
  protocol                 = "tcp"
  security_group_id        = aws_security_group.client_ecs_sg.id
  source_security_group_id = aws_security_group.allow_client_ecs_to_client_rds.id
}
...
resource "aws_security_group_rule" "client_rds_egress_response" {
  type                     = "egress"
  from_port                = 1024
  to_port                  = 65535
  protocol                 = "tcp"
  security_group_id        = aws_security_group.allow_client_ecs_to_client_rds.id
  source_security_group_id = aws_security_group.client_ecs_sg.id
  description              = "Allow response traffic to ECS clients only"
}
...
resource "aws_security_group_rule" "account_rds_egress_response" {
  type                     = "egress"
  from_port                = 1024
  to_port                  = 65535
  protocol                 = "tcp"
